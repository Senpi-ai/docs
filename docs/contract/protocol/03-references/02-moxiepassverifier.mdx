...
### isMoxiePassHolder (internal)

Checks if the caller is a Moxie Pass holder (internal function). If the caller does not hold the NFT, the function reverts with the error `NotaMoxiePassHolder`. If the erc721ContractAddress is not set, the function does not perform the check.
