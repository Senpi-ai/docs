...
Once your environment is set up, fork the repository and then clone it locally to your machine.
...
Then, copy the `packages/_examples/plugin` directory, which contain templates to create a new skills for your Creator Agent, to a new folder under the `packages` directory:

```sh
cp ./packages/_examples/plugin ./packages/plugin-first-skill
```

Once you created a new folder that will contain the code for your new Creator Agent skills, go to `packages/plugin-first-skill/src/samplePlugins.ts` to define your plugin instance:

```ts title="packages/plugin-first-skill/src/samplePlugins.ts"
...
```ts title="packages/plugin-first-skill/src/plugins/samplePlugins.ts" {8,9}
...
import myFirstCreatorAgentSkill from "@elizaos/plugin-first-skill";
