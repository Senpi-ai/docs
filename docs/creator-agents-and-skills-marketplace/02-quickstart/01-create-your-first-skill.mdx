...
```ts title="src/plugins/myFirstMoxieSkill.ts"
...
```ts title="src/providers/moxieSummaryProvider.ts"
import {
  type Provider,
  type IAgentRuntime,
  type Memory,
...
```ts title="src/actions/moxieSummaryAction.ts"
import type {
  Action,
  HandlerCallback,
  IAgentRuntime,
...
```ts title="src/actions/moxieSummaryAction.ts" {12}
import type {
  Action,
  HandlerCallback,
  IAgentRuntime,
...
```ts title="src/actions/moxieSummaryAction.ts" {14}
import type {
  Action,
  HandlerCallback,
  IAgentRuntime,
...
```ts title="src/plugins/index.ts" {8,9}
