...
Simply fork [this Dune table](https://dune.com/queries/4516789). Then, delete the existing query and replace with the following query below and `<moxie-user-id>` added as the desired user to search:
...
WITH gas_spent AS (
    ...
    WHERE
        DATE_TRUNC('day', block_time) = CURRENT_DATE
        AND blockchain = 'base'
    GROUP BY
        1, tx_from
), user_score AS (
