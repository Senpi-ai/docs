...
Auction orders are orders registered on the auction contract to bid on a [Creator Coin auction (FTA)](#creator-coin-auctions). The auction orders from multiple users will determine the clearing price of a Creator Coin once the auction is cleared/finalized.
...
Subject token is equivalent/synonymous to [Creator Coin](#creator-coin). This term is more commonly used in subgraphs and contracts.

## Portfolio

Portfolio is defined as all the [Creator Coins](#creator-coin) that a user owns. To get a user's portfolio, follow this tutorial [here](../use-cases/users/01-get-all-fan-tokens-owned-by-users.mdx).

## Vault

Vault is a smart contract that holds the [Moxie token](https://build.moxie.xyz/the-moxie-protocol/moxie-token) when [Creator Coins](#creator-coin) are purchased. On the other hand, if a [Creator Coin](#creator-coin) is sold, then the [Moxie token](https://build.moxie.xyz/the-moxie-protocol/moxie-token) will be withdrawn from the Vault contract.
