...
Senpi Agent Libs provides a suite of functions that enables developers to execute on-chain transactions and retrieve Senpi-related data efficiently.
...
const wallet: MoxieWalletClient = state.agentWallet;
const { signature, encoding } = await wallet.signMessage("Hello Senpi!");
```
...
## Fetch Senpi User Info By Wallet address

Fetch the Senpi User Info by providing the user's wallet address.
...
## Fetch Senpi User Info By Senpi User ID

Fetch the Senpi User Info by providing the user's Senpi User ID.
...
## Fetch Senpi User Info By Multiple Senpi User IDs

Fetch the Senpi User Info by providing an array of Senpi User IDs.
...
## Fetch Senpi User Info By Multiple Wallet Addresses

Fetch the Senpi User Info by providing an array of wallet addresses.
...
## Fetch Twitter User Names By Multiple Senpi User IDs

Fetch the Twitter user names by providing the multiple users' Senpi User ID.
...
## Fetch Social Profiles By Multiple Senpi User IDs

Fetch the social profiles by providing the multiple users' Senpi User ID.
...
## Fetch Portfolio By Senpi ID Order By TVL

Fetch the Senpi creator coin portfolio of a user by providing the user's Senpi User ID and the maximum number of coins to fetch.
