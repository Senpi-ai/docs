...
If the Moxie token `allowance` is either 0 or less than what is needed to buy the inputted amount of Fan Tokens (from [Step 2](./01-buy-fan-tokens.mdx#step-2-check-moxie-token-allowance)), then approval on the Moxie Token will need to be initiated to ensure that the buy transaction is successful.

Otherwise, if there is sufficient `allowance` value, skip to [Step 5](./01-buy-fan-tokens.mdx#step-5-execute-contract-call) to execute the buy Fan Tokens transaction.
...
Lastly, call the `buyShares` function on the Moxie bonding curve contract to execute the Fan Token purchase transaction:
