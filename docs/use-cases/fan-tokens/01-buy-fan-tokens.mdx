...
description: Learn how to make calls to the the Moxie Bonding Curve smart contract to buy Creator Coins, either from your wallet or your vesting contract.
---
...
If the Moxie token `allowance` is either 0 or less than what is needed to buy the inputted amount of Creator Coins (from [Step 2](./01-buy-fan-tokens.mdx#step-2-check-moxie-token-allowance)), then approval on the Moxie Token will need to be initiated to ensure that the buy transaction is successful.

Otherwise, if there is sufficient `allowance` value, skip to [Step 5](./01-buy-fan-tokens.mdx#step-5-execute-contract-call) to execute the buy Creator Coins transaction.
...
Lastly, call the `buySharesV2` function on the Moxie bonding curve contract to execute the Creator Coin purchase transaction:
