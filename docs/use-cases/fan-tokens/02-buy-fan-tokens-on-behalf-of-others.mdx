...
description: Learn how to make calls to the the Moxie Bonding Curve smart contract to buy Creator Coins on behalf of other address.
---
...
# Buy Creator Coins On Behalf Of Others

In this tutorial, you will learn step-by-step on how to buy any Creator Coins on behalf of an third-party address on the Moxie protocol through the bonding curve contract.
...
Starting 22nd November 2024, any EOA wallet or contracts on [Base](https://base.org/) can buy Creator Coins without needing to hold any [Moxie Pass](../../learn/glossary.mdx#moxie-pass).
...
If the Moxie token `allowance` is either 0 or less than what is needed to buy the inputted amount of Creator Coins (from [Step 2](./02-buy-fan-tokens-on-behalf-of-others.mdx#step-2-check-moxie-token-allowance)), then approval on the Moxie Token will need to be initiated to ensure that the buy transaction is successful.

Otherwise, if there is sufficient `allowance` value, skip to [Step 5](./02-buy-fan-tokens-on-behalf-of-others.mdx#step-5-execute-contract-call) to execute the buy Creator Coins transaction.
...
Lastly, call the `buySharesForV2` function on the Moxie bonding curve contract to execute the Creator Coin purchase transaction:
...
try {
  client.writeContract({
    ...
    args: [
      ...
      // the 3rd party address that will receive the Creator Coin
...
Congrats! 🥳🎉 You've just successfully purchased Creator Coin for a third-party address on Base!
