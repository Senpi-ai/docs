...
In this tutorial, you will learn step-by-step on how to sell any Fan Tokens on the Moxie protocol through the bonding curve contract.
...
If the Fan token `allowance` is either 0 or less than what is needed to sell the inputted amount of Fan Tokens (from [Step 2](./03-sell-fan-tokens.mdx#step-2-check-fan-token-allowance)), then approval on the Fan Token will need to be initiated to ensure that the buy transaction is successful.

Otherwise, if there is sufficient `allowance` value, skip to [Step 5](./03-sell-fan-tokens.mdx#step-5-execute-contract-call) to execute the buy Fan Tokens transaction.
...
Lastly, call the `sellSharesV2` function on the Moxie bonding curve contract to execute the Fan Token selling transaction:
...
Congrats! 🥳🎉 You've just successfully sold a Fan Token!
