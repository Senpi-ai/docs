...
description: Learn how to make calls to the the Moxie Bonding Curve smart contract to sell Creator Coins, either from your wallet or your vesting contract.
---
...
In this tutorial, you will learn step-by-step on how to sell any Creator Coins on the Moxie protocol through the bonding curve contract.
...
If the Creator Coin `allowance` is either 0 or less than what is needed to sell the inputted amount of Creator Coins (from [Step 2](./03-sell-fan-tokens.mdx#step-2-check-creator-coin-allowance)), then approval on the Creator Coin will need to be initiated to ensure that the buy transaction is successful.

Otherwise, if there is sufficient `allowance` value, skip to [Step 5](./03-sell-fan-tokens.mdx#step-5-execute-contract-call) to execute the buy Creator Coins transaction.
...
Lastly, call the `sellSharesV2` function on the Moxie bonding curve contract to execute the Creator Coin selling transaction:
...
Congrats! 🥳🎉 You've just successfully sold a Creator Coin!
