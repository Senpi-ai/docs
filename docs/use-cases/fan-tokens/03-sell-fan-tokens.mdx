...
If the Creator Coin `allowance` is either 0 or less than what is needed to sell the inputted amount of Creator Coins (from [Step 2](./03-sell-fan-tokens.mdx#step-2-check-fan-token-allowance)), then approval on the Creator Coin will need to be initiated to ensure that the buy transaction is successful.
