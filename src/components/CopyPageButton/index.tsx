...
export default function CopyPageButton(): JSX.Element {
  ...
  const getEditUrl = () => {
    ...
    if (process.env.NODE_ENV === "development") {
      ...
      let editUrl =
        ...
        siteConfig.themeConfig?.editUrl;
      ...
      if (editUrl) {
        ...
        const baseEditUrl = editUrl.endsWith("/")
          ? editUrl.slice(0, -1)
          : editUrl;
  ...
  const getRawUrl = (url: string): string | null => {
    ...
    const githubEditRegex =
      /github\.com\/([^/]+)\/([^/]+)\/(edit|tree)\/([^/]+)\/(.+)/;
  ...
  const viewAsMarkdown = async () => {
    ...
    if (rawUrl) {
      ...
      if (
        ...
        !currentEditUrl.startsWith("http")
      ) {
        try {
  ...
  const openInChatGPT = () => {
    ...
    const chatGptUrl = `https://chatgpt.com/?q=${encodeURIComponent(
      `Tell me about this page: ${currentUrl}`
    )}`;
  ...
  return (
    <div className={styles.copyPageButtonContainer} ref={dropdownRef}>
      ...
      {copySuccess && (
        <div className={styles.copySuccessTooltip}>Copied to clipboard!</div>
      )}
